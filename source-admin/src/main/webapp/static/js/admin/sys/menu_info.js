...
MenuInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/system/menu/add", function (data) {
       if(data.code==200){
    	   Feng.success("添加成功!");
...
MenuInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/system/menu/edit", function (data) {
    	if(data.code==200){
    		  Feng.success("修改成功!");
    	      window.parent.Menu.table.refresh();
    	      MenuInfoDlg.close();
...
$(function () {
    ...
    var ztree = new $ZTree("pcodeTree", "/system/menu/selectMenuTreeList");
