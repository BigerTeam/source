...
RolInfoDlg.addSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/system/role/add", function (data) {
        if(data.code==200){
        	Feng.success("添加成功!");
...
RolInfoDlg.editSubmit = function () {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/system/role/edit", function (data) {
    	if(data.code==200){
    		 Feng.success("修改成功!");
    	     window.parent.Role.table.refresh();
    	     RolInfoDlg.close();
...
$(function () {
    ...
    var deptTree = new $ZTree("deptTree", "/system/dept/tree");
    deptTree.bindOnClick(RolInfoDlg.onClickDept);
    deptTree.init();
    RolInfoDlg.deptZtree = deptTree;

    var pNameTree = new $ZTree("pNameTree", "/system/role/roleTreeList");
