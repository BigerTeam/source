...
DeptInfoDlg.addSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/system/dept/add", function(data){
       if(data.code==200){
    	   Feng.success("添加成功!");
...
DeptInfoDlg.editSubmit = function() {
    ...
    //提交信息
    var ajax = new $ax(Feng.ctxPath + "/system/dept/update", function(data){
       if(data.code==200){
       		Feng.success("修改成功!");
           window.parent.Dept.table.refresh();
           DeptInfoDlg.close();
...
$(function() {
    ...
    debugger
    var ztree = new $ZTree("parentDeptMenuTree", "/system/dept/tree");
