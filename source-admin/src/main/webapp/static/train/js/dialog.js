/**
 * dialog是一个简单易用但是功能强大的开源JS弹出窗口，具有很强的扩展性和兼容性，兼容IE6.0，目前版本1.2.
  包括锁屏对象dialog.lock， 提示工具 dialog.tip， 确认框 dialog.confirm  弹出窗口 dialog.win 比artDialog功能更强大，使用更方便。
 * @dependence jquery
...
dialog.lock = {
	...
	setSize : function() {
		...
		$(this.lockBak).css({
			...
			'width' : dialog.getDocWidth(),
			'height' : dialog.getDocHeight()
		});
	},
	...
	show : function() {
		if ( this.lockBak == null ) return;
		$(this.lockBak).fadeIn(dialog.transitionTime);
	},

	//隐藏锁屏
	hide : function() {
		if ( this.lockBak == null ) return;
		$(this.lockBak).fadeOut(dialog.transitionTime);
	},
	...
	work : function( options ) {
		...
		if ( this.options.timer > 0 ) {
			setTimeout(function() {
				dialog.lock.fadeOut(dialog.transitionTime);
			}, this.options.timer);
		}
...
dialog.tip = {
	...
	create : function() {
		...
		var T_ICON = $('<div class="jtip_left_icon" id="dialog_tip_icon"><!--提示图标--></div>');
		var T_CONTENT = $('<div class="jtip_content" id="dialog_tip_content"></div>');
	...
	setContent : function() {
		...
		$('#dialog_tip_content').html(this.options.content);
	...
	center : function() {
		...
		if ( this.options.width ) {
			...
			var b_width = $('#dialog_tip_icon').width() + $('#jtip_loading').width() + 21 + $('#jtip_right').width();
			_width = b_width + $('#dialog_tip_content').width();

		}
		var top = (dialog.getWindowHeight() - $(this.tipBak).height())/2 + _scrollTop;
		...
		$(this.tipBak).css({
			'width' : _width +'px',
			'top'   : top + 'px',
			'left'  : (dialog.getWindowWidth() - _width)/2 + 'px'
		});
	},
	...
	show : function () {
		...
		switch ( this.options.type ) {
			case 'warn':
				$('#dialog_tip_icon').addClass('jtip_warn');
				break;
			case 'ok':
				$('#dialog_tip_icon').addClass('jtip_ok');
				break;
			case 'error':
				$('#dialog_tip_icon').addClass('jtip_error');
				break;
			case 'loading':
				var _loading = $('<div id="jtip_loading" class="jtip_load_img"><em id="jtip_loading_icon"></em></div>');
				var _tip_icon = $('#dialog_tip_icon');
...
var __JWindow__ = function( __options ) {
	...
	o.create = function() {
		...
		o.winBox = $('<div class="dialog_win_box dialog_win_'+options.skin+'"></div>');
		if ( options.borderWidth > 0 ) {
			o.winBg = $('<div class="dialog_win_box_bg"></div>');
		}

		//line 1, create title of window
		var title_box = $('<div class="dialog_win_title_box"></div>');
		var win_title = $('<div class="dialog_win_title">'+options.title+'</div>');

		var win_button = $('<div class="dialog_win_button"></div>');
		var max_button = $('<a class="dialog_win_max_button" href="javascript:void(0)" title="最大化"></a>');
		var close_button = $('<a class="dialog_win_close_button" href="javascript:void(0)" title="关闭"></a>');
		...
		var win_content = $('<div class="dialog_win_CBOX"></div>');
		//add icon
		var icon = $('<span class="dialog_win_icon"></span>');
		var content = $('<div class="dialog_win_content">'+options.content+'</div>');
		...
		var btnBox = $('<div class="dialog_win_button_container"></div>');
		var btn = $('<div class="dialog_win_buttonInner"></div>');
	...
	o.setIcon = function( itype ) {
		...
		o.winBox.find('.dialog_win_icon').addClass("dialog_win_icon_"+itype);
	}
	...
	o.setPosition = function() {
		...
		if ( options.position == 'center' ) {
			...
			top = (dialog.getWindowHeight() - height)/2 + scroll_top;
			left = (dialog.getWindowWidth() - options.width)/2;
		}
		...
		//调整内容宽度
		if ( options.icon == "none" ) {
			o.winBox.find(".dialog_win_content").css({width:"100%"});
		} else {
			o.winBox.find(".dialog_win_content").css({width: (o.winBox.width() - 20 - o.winBox.find(".dialog_win_icon").width() - 15) + "px"});
		}
	...
	o.show = function() {
		...
		switch ( options.effect ) {
			...
			case 0:
				o.winBox.fadeIn(dialog.transitionTime);
				if ( o.winBg != null ) {
					o.winBg.fadeIn(dialog.transitionTime);
				}
				break;

			case 1:
				o.winBox.flyIn(dialog.transitionTime);
				if ( o.winBg != null ) {
					o.winBg.flyIn(dialog.transitionTime);
				}
				break;

			case 2:
				o.winBox.zoomIn(dialog.transitionTime);
				if ( o.winBg != null ) {
					o.winBg.zoomIn(dialog.transitionTime);
				}
				break;
		}
	}
	...
	o.clear = function() {
		if ( o.winBox == null ) return;
		o.winBox.find('.dialog_win_CBOX').empty();
		o.setPosition();
	}
	...
	o.resizeToMax = function() {
		...
		if ( data == undefined ) {
			...
			//设置还原按钮属性
			max_btn.attr({
				'class' : 'dialog_win_reduce_button',
				'title' : '还原'
			});
	...
	o.close = function() {
		...
		if ( o.winBox != null ) {
			...
			switch ( options.effect ) {
				...
				case 0:
					o.winBox.fadeOut(dialog.transitionTime);
					if (o.winBg != null ) {
						o.winBg.fadeOut(dialog.transitionTime);
					}
					break;

				case 1:
					o.winBox.flyOut(dialog.transitionTime);
					if (o.winBg != null ) {
						o.winBg.flyOut(dialog.transitionTime);
					}
					break;

				case 2:
					o.winBox.zoomOut(dialog.transitionTime);
					if (o.winBg != null ) {
						o.winBg.zoomOut(dialog.transitionTime);
					}
					break;
			}
